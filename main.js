...
class App extends Quiz {
  ...
  get renderAnswer() {
    ...
    for (const [answer, text] of Object.entries(answers)) {
      const answerContainer = document.createElement("div"),
        ...
        checked =
          (multi && userAnswer?.includes(answer)) || userAnswer === answer;
  ...
  get renderAction() {
    ...
    const saveChoiceAnswers = () => {
      super.saveChoice = Array.from(document.querySelectorAll("input:checked"))
  ...
  get renderQuizResults() {
    ...
    container.className = "quiz-result-container";
      ...
      for (const [key, answer] of Object.entries(answers)) {
        ...
        if (multi) {
          ...
          for (const letter of letters) {
            const isUserSelected = key === letter;
            const isUserCorrect = correctAnswer.includes(letter);

            if (isUserSelected) {
              answerElement.classList.add("selected");
              answerComment.textContent = "Your answer";
              markcontainer.textContent = isUserCorrect ? "✔" : "✖";
          ...
          const isUserSelected = key === this.userAnswers[index];
          const isUserCorrect = this.userAnswers[index] === correctAnswer;
